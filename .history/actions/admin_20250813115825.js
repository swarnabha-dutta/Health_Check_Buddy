 ...
 */
export async function updateDoctorActivityStatus(formData) {
    const isAdmin = await verifyAdmin();
    if (!isAdmin) throw new Error("Unauthorized");
...
export async function getPendingPayouts() {
    ...
    try {
        const pendingPayouts = await db.user.findMany({
            where: {
                status: "PROCESSING",
            },
            include: {
...
export async function approvePayout(formData) {
    ...
    try {
        ...
        if (!payout) throw new Error("Payout request  not found or already approved");
        
        // Check if doctor has enough credits
        if (payout.doctor.credits < payout.credits) {
            throw new Error("Doctor does not have enough credits to approve this payout request");
        }
